...
## Live provenance + P0 Search Quality v2 preflight (2026-08-15)

Live baseline has not started. API, worker, PostgreSQL, OpenSearch, MinIO,
parser services and Model Gateway returned `ok`, but it is not a valid
local-private model deployment: the healthy active aliases are OpenRouter
(`embed_default`, `generator_fast`, `generator_main`, `verifier`,
`rerank_default`), while the available local aliases are unhealthy.

Search projection is also not clean: two historical reconciliation rows for
`verify-document.pdf` and `verify-metadata.csv` remain `degraded` after 23
attempts with safe code `INTERNAL_ERROR`; the worker repeatedly renews their
leases without a terminal reconciliation result.  This is a baseline blocker,
not an acceptable warning.  No signing secret, temporary KB, upload, v2
corpus, binding, dev run or test run was created.  The required next action is
to provision and activate healthy local embedding/rerank/chat Gateway aliases,
then repair or root-cause those two reconciliation records before beginning
the controlled smoke.

## Source provenance contract v1 (2026-08-15)

Direct upload now accepts the typed `source_ref_v1` contract on session,
batch and multipart paths.  Server-owned `(tenant, KB, namespace,
external_id)` identity determines `document_id`; content or source-version
changes create a `document_version`, while an unchanged version remains
idempotent.  External identity is persisted through `knowledge_sources` and
`source_document_states`; legacy IDs are compatibility projections and no
longer silently rebind.

`source_provenance_v1` is generated once and projected in document viewer,
search, answer evidence and Extended/Deep Research evidence.  User attributes
are bounded and cannot override identity, ACL, storage, checksum or filename.
The quality-ingest path supplies an eval namespace/source reference.  A signed
runtime-binding v2 builder resolves frozen logical quotes via authorized
document context responses and rejects stale, tampered, unresolved and
ambiguous evidence.  Creating an actual binding requires the local secret
`WIKIPEDIARAG_EVAL_BINDING_SIGNING_KEY`; no existing P0.1 artifact or baseline
was changed, and a new `p0-search-quality-v2` material set must be provisioned
and frozen before its 44/176 baseline is run.

Focused verification: `29 passed` for provenance/upload/chunker/eval-binding
and quality unit tests; `test_search_service.py` (7) and
`test_document_viewer.py` (10) passed; focused Ruff and Mypy passed.  No live
upload/worker/publication/search run was executed in this change.

## P0.1 — расширение правил измерения качества поиска (2026-08-15)

Добавлен единый путь проверки качества в `src/wikipediarag/eval`: новые поля
описания вопроса и источника, безопасная запись шагов, ключ сопоставимости,
расчёты по видам вопросов и языкам, продолжение запуска и команды
`eval-quality-*`. Старые наборы Wikipedia и RRNCB продолжают читаться с
прежними значениями по умолчанию.

В локальной папке `eval-corpus/p0-search-quality-v1/` подготовлен и закреплён
контрольный набор из 220 вопросов: 11 видов по 20 вопросов, пять языков по 44
вопроса, части `dev=44` и `test=176`. Папка добавлена в `.gitignore`; её
контрольные суммы входят в описание набора. Этот набор создан как безопасная
детерминированная проверка пути и не является ручным подтверждением настоящего
набора Wikipedia/RRNCB. Для производственного замера нужно заменить локальные
файлы на утверждённые материалы и повторить ручную проверку и загрузку.

Фактические проверки 2026-08-15:

- `uv run pytest tests/unit` — 525 passed, 2 предупреждения;
- `uv run ruff check .` — passed;
- `uv run ruff format --check .` — 170 файлов без изменений;
- `uv run mypy src tests` — passed;
- `uv run pytest tests/unit/test_eval_quality.py tests/unit/test_eval_metrics.py tests/unit/test_eval_review.py tests/unit/test_eval_retrieval_runner.py -q` — 26 passed;
- подготовка, просмотр и закрепление P0.1 — 220 проверенных строк, `dev=44`, `test=176`;
- `make test-functional-retrieval` не запустилась: в среде Windows нет команды `make`;
  это отдельная проверка рабочего поиска и не меняет результат замера P0.1.
...
Полный технический исходный замер выполнен на локальном имитационном поставщике
моделей. Сначала API и шлюз модели вернули `ok`, затем команда загрузки
`eval-quality-ingest` обработала 20 из 20 файлов и создала пять отдельных баз
проверки. Запуск `p01-quality-20260815-mock-v3` выполнил последовательно:

- `dev`: 44 из 44 вопросов, 44 поиска и 44 ответа, ошибок 0;
- `test`: 176 из 176 вопросов, 176 поисков и 176 ответов, ошибок 0;
- всего: 220 из 220 вопросов, ошибок выполнения 0.

Закреплённый набор имеет контрольную сумму
`fc2c379f01b60347988b746b944d55ab269c409b7cd277a3ac33dbdfa1bab1b0`.
Загрузка сохранена в
`eval-corpus/p0-search-quality-v1/ingestion/p01-ingest-20260815-mock.json`,
сводка — в `artifacts/eval/quality/p0-search-quality-v1/latest.json`,
отдельные записи поиска и ответа — в папке запуска
`artifacts/eval/quality/p0-search-quality-v1/p01-quality-20260815-mock-v3/`.

Сводка помечена `incompatible_results`: пять языковых баз имеют разные номера
индекса, поэтому общий средний показатель намеренно не считается; результаты
разделены по языку, виду вопроса и базе. Это ожидаемое защитное поведение для
текущего отдельного размещения баз, а не ошибка выполнения. Значения качества
в этом замере нельзя принимать за производственный показатель: локальный набор
содержит безопасные вымышленные материалы, а эталонные идентификаторы вопросов
ещё не связаны с внутренними идентификаторами документов после загрузки.

Дополнительная попытка передать все пять баз одним поисковым запросом
(`p01-quality-20260815-mock-v4`) остановилась на ошибке рабочего много-базового
пути `KeyError: stage`; этот эксперимент не включён в исходный замер и требует
отдельной задачи вне P0.1.
...
- Historical status archive: [history/STATUS-archive.md](STATUS-archive.md).
